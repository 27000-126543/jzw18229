...
import { TrendingUp, DollarSign, Download, Users, Package, Edit, Trash2, Plus } from 'lucide-react'
...
const stats = [
  { icon: TrendingUp, label: '总销量', value: 3560, prefix: '', suffix: '', change: '+12.5%' },
  { icon: DollarSign, label: '总收入', value: 178000, prefix: '¥', suffix: '', change: '+8.3%' },
  { icon: DollarSign, label: '月收入', value: 22400, prefix: '¥', suffix: '', change: '-5.2%' },
  { icon: Download, label: '总下载', value: 18900, prefix: '', suffix: '', change: '+15.1%' },
]
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="glass-card p-6">
        <h3 className="section-title text-xl mb-6">下载趋势</h3>
        <div className="h-72">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={mockData.downloadTrend}>
              ...
              <Line type="monotone" dataKey="downloads" stroke="#FF6B2C" strokeWidth={2} dot={{ fill: '#FF6B2C', r: 4 }} />
              <Line type="monotone" dataKey="sales" stroke="#FFA266" strokeWidth={2} dot={{ fill: '#FFA266', r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
            ...
            <tbody>
              {topProducts.map((p) => (
                <tr key={p.id} className="border-b border-surface-800/50 hover:bg-surface-800/30 transition">
                  ...
                  <td className="py-3 text-surface-300 text-right">¥{((p.salesCount * p.pricePersonal) || 0).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function ProductsTab({ products }: { products: Product[] }) {
  const statusMap: Record<string, { label: string; cls: string }> = {
    ...
    pending: { label: '审核中', cls: 'bg-yellow-500/15 text-yellow-400' },
              ...
              {products.map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-b border-surface-800/50 hover:bg-surface-800/30 transition">
                    ...
                    <td className="px-4 py-4 text-surface-300">{p.rating}</td>
                    <td className="px-4 py-4"><span className={`badge ${st.cls}`}>{st.label}</span></td>
                    <td className="px-4 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button className="p-2 rounded-lg hover:bg-surface-700 text-surface-400 hover:text-brand-400 transition"><Edit className="w-4 h-4" /></button>
                        <button className="p-2 rounded-lg hover:bg-surface-700 text-surface-400 hover:text-red-400 transition"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                )
...
function SettlementsTab() {
  ...
  const totalEarned = settlements.reduce((s, r) => s + r.totalRevenue, 0)
  ...
  const statusMap: Record<string, { label: string; cls: string }> = {
    ...
    processing: { label: '处理中', cls: 'bg-yellow-500/15 text-yellow-400' },
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: '总收入', value: totalEarned, icon: DollarSign },
          { label: '平台手续费(15%)', value: totalFees, icon: Users },
          { label: '实际到账', value: totalNet, icon: TrendingUp },
        ].map((c) => (
